  ...
  return (
    <div className={className ? `menu ${className}`: "menu"}>
      ...
      onClick={toggleMenu}>
        ...
        <ul>
        {
                menulinks.map(({url,title},index)=>{
                        return <li key={index} className={(location.pathname=='/' && title=="The official guide to Copenhagen") ? 'active-link' : 'non-active'}><NavLink to={ url } className="menu-hover">{ title }</NavLink></li>
                })
        }
      </ul>
